...
cv2.createTrackbar("Update Threshold", "Settings", 5, 300, lambda x: None)  # Threshold for updating positions
...
def identify_outermost_inner_and_cell_centers(grid_intersections, last_grid, last_centers, update_threshold):
    """Identifies outermost intersections (green), corners (orange), inner intersections (blue), and cell centers (red)."""
    ...
    for i in range(len(grid_intersections)):  # Iterate over rows
        for j in range(len(grid_intersections[i])):  # Iterate over columns
            ...
            if last_grid and np.linalg.norm(np.array(last_grid[i][j]) - np.array(point)) < update_threshold:
                point = last_grid[i][j]
    ...
    for i in range(len(grid_intersections) - 1):
        for j in range(len(grid_intersections[i]) - 1):
            ...
            # Apply update threshold check
            if last_centers and np.linalg.norm(np.array(last_centers[i * len(grid_intersections) + j]) - np.array((center_x, center_y))) < update_threshold:
                center_x, center_y = last_centers[i * len(grid_intersections) + j]

            cell_centers.append((center_x, center_y))

    return outermost, inner, corner_points, cell_centers
    ...
    while True:
        ...
        try:
            ...
            if structured_grid is not None:
                ...
                outermost, inner, corners, cell_centers = identify_outermost_inner_and_cell_centers(structured_grid, last_valid_grid, last_valid_centers, update_threshold)

                last_valid_grid, last_valid_centers = structured_grid, cell_centers

                for x, y in outermost + inner + corners:
                    cv2.circle(intersection_image, (x, y), 3, (0, 255, 0), -1)
