...
def find_cell_centers(grid):
    ...
    we can form (R-1)*(C-1) cells. Each cell center is
    the midpoint of diagonal corners:
        cell_center = ( (x00 + x11)//2 , (y00 + y11)//2 )
    But we must check row/col bounds in case rows have different lengths.
    """
    centers = []
    labels = []
    label_count = 1

    # We iterate until the second-to-last row, so row_idx+1 is valid.
    for row_idx in range(len(grid) - 1):
        # We'll figure out how many columns are in the current row
        num_cols_current = len(grid[row_idx])
        num_cols_next = len(grid[row_idx + 1])
        
        # The maximum valid col_idx for a "cell" is the min of
        # (num_cols_current, num_cols_next) - 2
        # so that col_idx+1 is in range for both rows.
        max_cols = min(num_cols_current, num_cols_next) - 1

        for col_idx in range(max_cols):
            # Now we know col_idx and col_idx+1 are valid in both rows
...
def main():
    ...
    while True:
        ...
        if cluster_dist < 1:
            cluster_dist = 1  # avoid division by zero or no clustering
        ...
        # The requirement says: "only update if new grid's distance > threshold"
        # So if the movement is bigger than threshold, we ACCEPT the new grid,
        ...
        if new_grid:
            if last_valid_grid is None:
                ...
                if dist > update_thresh:
                    # We accept the new grid if the change is big
                    last_valid_grid = new_grid
                # else: do nothing, keep last_valid_grid
        # If new_grid is empty or we didn't accept it, we do not change last_valid_grid
        
        output_frame = frame.copy()
        
        # If we have a valid grid, let's draw everything
        if last_valid_grid:
            # (1) Find cell centers
            ...
            # (3) If "Show Intersections" is on, color the intersection points
            #     4 corners -> orange, other outer intersections -> green, inner -> blue
            # We'll first identify #rows, #cols in the grid
            ...
            # (4) If "Show Lines" is on, draw pink lines connecting the outer (green) intersections
        if show_lines == 1:
            num_rows = len(last_valid_grid)
            
            # --- A. Draw all horizontal lines in each row ---
            ...
            # --- B. Draw all vertical lines (connecting row i to row i+1) ---
        ...
        # Show the "Output Feed" in a window
