...
cv2.createTrackbar("Clustering Distance", "Settings", 5, 50, lambda x: None)  # Distance to group nearby intersections
...
def identify_outermost_intersections(grid_intersections):
    """Identifies and marks outermost intersections separately."""
...
if not cap.isOpened():
    ...
    while True:
        ...
        if len(structured_grid) > 0:
            outermost_intersections, inner_intersections = identify_outermost_intersections(structured_grid)
            ...
            cv2.imshow("Detected Intersections (Blue = Inner, Green = Outer)", intersection_image)
