...
START_COLOR = (0, 255, 0)        # Green for the start node
TARGET_COLOR = (255, 0, 0)       # Red for the target node
...
def show_no_solution_message():
    ...
    # Restart the game
    global grid
    grid = generate_grid()  # Regenerate grid
...
def main():
    global grid
    grid = generate_grid()  # Initial grid generation
    ...
    while running:
        if needs_update:
            update_frame(grid, start, target, visited, path)  # Update grid visualization
