...
cv2.createTrackbar("Clustering Distance", "Settings", 90, 200, lambda x: None)  # Base clustering distance set to 50, adjustable up to 200
...
def identify_outermost_and_inner_intersections(grid_intersections):
    """Identifies outermost intersections (green), corners (orange), and inner intersections (blue)."""
    ...
    while True:
        ...
        try:
            ...
            if len(structured_grid) > 0:
                outermost_intersections, inner_intersections, corner_intersections = identify_outermost_and_inner_intersections(structured_grid)
                ...
                for row in structured_grid:
                    for i in range(len(row) - 1):
                        cv2.line(intersection_image, row[i], row[i + 1], (0, 255, 0), 2)  # Green horizontal connections

                for col in range(len(structured_grid[0])):
                    for i in range(len(structured_grid) - 1):
                        cv2.line(intersection_image, structured_grid[i][col], structured_grid[i + 1][col], (0, 255, 0), 2)  # Green vertical connections
